...
#(
  ...
  output softex_config_t  config_o,
  ...
  softex_config_t [XifNumHarts-1:0] config_d, config_q;

  always_comb begin : legal_inst_assignment
    legal_inst = 1'b0;

    unique case ({x_issue_req_i.instr[14:12],x_issue_req_i.instr[6:0]})
      {MainFunct3,Opcode}, {SlotFunct3,Opcode}, {WghtFunct3,Opcode}: legal_inst = 1'b1;
  ...
  assign config_valid_o = ~issue_fifo_empty[winner] && ~register_fifo_empty[winner] && x_result_ready_i && cur_issue[winner].instr[6:0] == Opcode && cur_issue[winner].instr[14:12] == MainFunct3;
  ...
  for (genvar i = 0; i < XifNumHarts; i++) begin : gen_instruction_fifos
    ...
    always_comb begin : config_assignment
      ...
      unique case ({cur_issue[i].instr[14:12],cur_issue[i].instr[6:0]})
        {MainFunct3,Opcode}: begin
          config_d[i].in_addr         = cur_register[i].rs[0][31:0];
          config_d[i].out_addr        = cur_register[i].rs[1][31:0];
          config_d[i].tot_len         = cur_register[i].rs[2][31:0];
          config_d[i].acc_only        = cur_issue[i].instr[7];
          config_d[i].div_only        = cur_issue[i].instr[8];
          config_d[i].last            = cur_issue[i].instr[9];
          config_d[i].gelu_mode       = cur_issue[i].instr[10];
          config_d[i].acquire_slot    = cur_issue[i].instr[11];
          config_d[i].set_cache_addr  = cur_issue[i].instr[25];
          config_d[i].no_op           = cur_issue[i].instr[26];
        end
        {SlotFunct3,Opcode}: begin
          config_d[i].cache_base_addr = cur_register[i].rs[0][31:0];
          config_d[i].slot            = cur_register[i].rs[1][15:0];
        end
        {WghtFunct3,Opcode}: begin
          config_d[i].a_addr          = cur_register[i].rs[0][31:0];
          config_d[i].b_addr          = cur_register[i].rs[1][31:0];
          config_d[i].weight_len      = cur_register[i].rs[2][31:0];
        end
      endcase
    end
  end

endmodule: softex_inst_decoder
